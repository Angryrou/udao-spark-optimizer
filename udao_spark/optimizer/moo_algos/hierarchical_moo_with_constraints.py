...
class Hierarchical_MOO_with_Constraints:
    ...
    def __init__(
        ...
        Including the number of subQs, the initial samples of theta_c, theta_p, theta_s and the random seed used
    ...
    def model_setup(
        ...
        :param non_decision_tabular_features: non-decision tabular features: shape (n_subQs, 28)
        :param obj_model: the function calling predictive models to return latency and cost values,
                with input of graph embeddings, non-decision tabular features, theta and model name
                (e.g. dict() = {"ana_latency_s": "WeightedEnsemble_L2_FULL", "io_mb":"CatBoost"})
    ...
    def hmooc_setup(
        ...
        '''
        initialize input related to the Hierarchical Mulit-Objective Optimization with Constraints (HMOOC) approach.
        :param theta_sample_mode: the mode to indicate how to get the initial theta samples, e.g. "grid-search", "random"
        :param theta_sample_funcs: the function passed to generate the "random" initial samples for theta.
                it takes inputs 1) the #samples (int), 2) type (str, e.g. "c" indicates for theta_c, similarly for "p" and "s"),
                3) seed (int), 4) whether to normalize (bool), and it returns an np.ndarray with shape (#samples, len_theta_c/p/s).
                If using AutoGulon,  normalize = False, otherwise (i.e. using MLP), normalize = True
        :param n_clusters: the number of clusters used to cluster theta_c samples
        :param cross_location: the location of crossover, e.g. the following is the case of cross_location=3
                ###############|##########################
        ...
        :param cluster_algo: the clustering method used to cluster theta_c samples, e.g. "k-means"
        :param runmode: to indicate whether using multiprocessing
        :param time_limit: the time limit with unit of seconds. e.g. 60, which means the function stops running at 60s
                            "-1" means no time limit.
        :param verbose: the flag to indicate whether to print more information during running
        '''
    ...
    def solve(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray, dict]:
        ...
        of all subQs, and the model inference information during running (e.g. the number of evaluations, the time cost)
        :return: query_obj_values: shape (#solutions, n_objs), here n_objs = 2, with latency and cost
                ...
                model_inference_info: shape (3, 2), where 3 means HMOOC calls the predictive model 3 times; 2 shows two
                                      columes "n_evaluations" and "inference_time" respectively,
    ...
    def _get_subQ_sets(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        :return: obj_values_all_subQs: the subQ-level Pareto optimal solutions, which are Pareto optimal under each
                            theta_c and subQ, shape (#total_solutions, n_objs), here n_objs=2 with latency and cost.
                config_all_subQs: the corresponding configurations, shape (#total_solutions, len_one_theta), here
                len_one_theta = len_theta_c + len_theta_p + len_theta_s = 19
                indices_all_subQs: the indices of subQs, theta_c and optimal theta_p for the obj_valeus_all_subQs,
                            with shape (#total_solution, 3),
                            e.g. np.array([[0, 1, 2]]) indicates the second solution of subQ0 under theta_c_1
                             (indexing from 0).
                final_model_inference_info: shape (3, 2), where 3 means HMOOC calls the predictive model 3 times; 2 shows two
                            columes "n_evaluations" and "inference_time" respectively,
                            e.g. when 100 * len_one_theta is fed in the model, n_evaluations = 100
        '''
        ...
        (obj_values_all_subQs_rep_theta_c, configs_all_subQs_rep_theta_c, indices_all_subQs_rep_theta_c,
         model_inference_info_optimize_theta_p), tc_optimize_theta_p_s = self._optimize_theta_p_and_s(
            representative_theta_c,
        ...
        (new_cluster_members, new_representative_theta_c), \
    ...
    def _dag_opt(
        ...
        :param indices_all_subQs: the indices of subQs, theta_c and optimal theta_p for the obj_valeus_all_subQs,
                            ...
                            e.g. np.array([[0, 1, 2]]) indicates the second solution of subQ0 under theta_c_1
                             (indexing from 0).
        :return: query_obj_values: the query-level Pareto optimal solutions, shape (#solutions, n_objs), here n_objs=2
                query_configurations: configurations of all subQs, shape (#solutions, len_one_theta * n_subQs)
        '''
    ...
    def clustering_method(
        ...
        e.g. 6 theta_c samples are clustered into 3 groups, np.array([0, 1, 1, 2, 0, 0])
        :param n_cluster: the number of clusters
        :param cluster_features: the samples to be clustered, with shape (n_samples, n_dim)
        ...
        if cluster_algo == "k-means":
            ...
            raise Exception(
                f"Clustering method {cluster_algo} is not supported!"
            )
    ...
    def cluster_representation(
        ...
        :param cluster_labels: cluster labels, where it includes n elements, and each represents a cluster label
        :return: the cluster representations (e.g. 3 clusters, [4, 1, 3])
                and the cluster members (e.g. [np.array([0, 4, 5)], np.array([1, 2]), np.array([3])])
        '''

        # # use the indices of cluster labels to indicate the indices of theta_c
        # uniq_cluster_labels = np.unique(cluster_labels)
        # # cluster_members: list
        # # indices of the list is the cluster label, each item is the indices of theta_c in the same cluster (np.adarray)
        # cluster_members = [np.where(cluster_labels == x)[0] for x in uniq_cluster_labels]
        #
        # # randomly choose one as the representative theta_c index of each cluster
        # np.random.seed(self.seed)
        # representative_theta_c = [np.random.choice(x, 1).item() for x in cluster_members]
    ...
    def construct_model_input_for_opt_p(
    ...
    ) -> Tuple[int, Union[np.ndarray, th.Tensor], th.Tensor, Union[th.Tensor, pd.DataFrame]]:
        ''' # fixme: double-check the refactoring
        construct the model input from the representative theta_c features, initial theta_p and theta_s samples,
        where the model input includes theta_c, theta_p, theta_s, graph_embeddings and non_decision_tabular_features.
        Graph embeddings and non_decision_tabular_features are with shape (n_subQs, x),
              which indicates that they are the same in the same subQ and different in different subQs;
        e.g. to get ONE predictive value of the subQ-level predictive model, it should include:
             one theta_c (1, 8), one theta_p (1, 9), one theta_s (1, 2), graph_embeddings (1, 32)
             and non_decision_tabular_feature (1, 28)
        The returned theta follows the orders of theta_c, theta_p and theta_s repsectively.
            e.g. given 2 theta_c_samples, 3 theta_p samples and 1 theta_s samples, the returned theta is:
            ################################
        ...
        :param n_subQs: the number of subQs
        :param use_ag: model flag, to indicate whether to use AutoGluon in the predictive model
        :param graph_embeddings: input of the model, with shape (n_subQs, 32)
        :param non_decision_tabular_features: input of the model, with shape (n_subQs, 28)
        :return: the number of evaluations fed in the model (i.e. how many predictions it generates),
                the full theta, graph_embeddings and non_decision_tabular_features for all subQs
        '''
        # the number of evaluations fed in the model
        n_evals = represent_theta_c_features.shape[0] * theta_p_samples.shape[0] * n_subQs
        ...
        if isinstance(represent_theta_c_features, np.ndarray):
            ...
            mesh_c_per_stage = np.repeat(represent_theta_c_features, theta_p_samples.shape[0], axis=0)
            mesh_c = np.tile(mesh_c_per_stage, (n_subQs, 1))

            mesh_p_per_stage = np.tile(theta_p_samples, (represent_theta_c_features.shape[0], 1))
            ...
            mesh_c = represent_theta_c_features.repeat_interleave(theta_p_samples.shape[0], dim=0).repeat(
                n_subQs, 1
            )
            mesh_p = theta_p_samples.repeat(represent_theta_c_features.shape[0], 1).repeat(n_subQs, 1)
        ...
        return n_evals, mesh_theta, mesh_graph_embeddings, mesh_non_decision_tabular_features
    ...
    def get_obj_values(
        ...
        :param mesh_non_decision_tabular_features: the non_decision_tabular_features for all subQs
    ...
    def filter_dominated_theta_p_s(
        ...
        for each subQ, filter dominated solutions under each theta_c, as theta_c should be the same among all subQs
        :param representative_theta_c: the theta_c indices of representative theta_c of theta_c clusters
        :param n_theta_p_samples: the number of initial theta_p samples
        :param n_subQs: the number of subQs
        :param mesh_theta: the input theta for all subQs,
               with shape (n_subQs * n_representative_theta_c * n_theta_p_samples, (len_theta_c+len_theta_p+len_theta_s)
        :param y_hat: the predictions with input mesh theta and graph embeddings and non-decision tabular features
        :param use_ag: model flag, to indicate whether to use AutoGluon in the predictive model
        :return: optimal solutions (subQ-level objective values and corresponding configurations) under each theta_c
                for all subQs

        '''

        # each element in the list is an array with shape (n_theta_p_samples, n_objs)
        # each element represents the objective values with fixed subQ and theta_c and different theta_p samples.
        # the length of the list is n_subQs * n_representative_theta_c
        # e.g. with 2 subQs and 3 representative theta_c, it splits into 6 elements, where each is an array
        ...
        (split_y_hat, split_theta), tc_split_theta = self._split_solutions(y_hat,
                                                         mesh_theta,
                                                         n_subQs,
        ...
        for i, (sub_obj_values, sub_config) in enumerate(zip(split_y_hat, split_theta)):
            ...
            # the columes are: subQ_id, theta_c_id, optimal_theta_p_id (from 0 to n_optimal_theta_p)
            optimal_indices_subQ_theta_c_p.append(opt_indices)

        if use_ag:
            assert isinstance(optimal_configurations_per_subQ_theta_c[0], np.ndarray)
            optimal_configurations_all_subQs = np.concatenate(optimal_configurations_per_subQ_theta_c)
        else:
            assert isinstance(optimal_configurations_per_subQ_theta_c[0], th.Tensor)
            optimal_configurations_all_subQs = th.concatenate(optimal_configurations_per_subQ_theta_c)

        optimal_obj_values_all_subQs = np.concatenate(optimal_obj_values_per_subQ_theta_c)
        indices_all_subQs_theta_c_p = np.concatenate(optimal_indices_subQ_theta_c_p)

        return optimal_obj_values_all_subQs, optimal_configurations_all_subQs, indices_all_subQs_theta_c_p
    ...
    def _split_solutions(
        ...
        ''' fixme: to double-check
        split the model predictions to generate a list, where each element has the same subQ_id and theta_c_id
        ...
        :return: split_y_hat: list, where each item is an array with shape (n_theta_p_samples, n_objs)
                split_theta: list, where each item is an array with shape (n_theta_p_samples, len_one_theta)
        '''
    ...
    def keep_opt_theta_p_per_theta_c(
        ...
        :param representative_theta_c: indices of representative theta_c in theta_c clusters
        :param sub_obj_values: the objective values under a fixed theta_c for a specified subQ
        :param sub_config: the configurations under a fixed theta_c for a specified subQ
        :return: po_objs: array, the Pareto optimal solutions
                po_configs: array, the corresponding configurations
                indices_arr: array, indices of subQ_id, theta_c_id and optimal theta_p_id
                    e.g. np.array([[0, 1, 0], [0, 1, 1], [0, 1, 2]])
                    it represents that for subQ0 with theta_c1, there are 3 optimal solutions
        '''
        # e.g. with 2 subQs and 3 representative theta_c, it splits into 6 elements, where each is an array
        ##          split_y_hat:                element  #
        ##################################################
    ...
    def _optimize_theta_p_and_s(
        ...
        ''' # fixme: to double-check
        optimize theta_p and theta_s (for simplicity, the following only mention theta_p as theta_p and theta_s function
         the same) under each fixed theta_c for all subQs
        :param representative_theta_c: the indices of representative theta_c in theta_c clusters
        :param representative_theta_c_features: the features of representative theta_c
        ...
        :return: optimal_obj_values_all_subQs: array, the optimal objective values for all subQs
                optimal_configurations_all_subQs: the corresponding configurations,
                    where the length of columes is len_one_theta
                indices_all_subQs_theta_c_p: the array, where columes show subQ_ids, theta_c_ids and ids of optimal
                    theta_p solutions
                model_inference_arr: the array of model inference info, where columes show [n_evals, model_inference_time]
                    e.g. n_evals is equal to the number of predictions/input from the model
        '''

        (n_evals, mesh_theta, mesh_graph_embeddings, mesh_non_decision_tabular_features), \
        tc_construct_model_input_for_opt_p = self.construct_model_input_for_opt_p(representative_theta_c_features,
                                                 theta_p_samples,
                                                 theta_s_samples,
                                                 n_subQs,
                                                 self.use_ag,
    ...
    def assign_optimal_theta_p_s(
            ...
            non_decision_tabular_features: Union[th.Tensor, pd.DataFrame],
            mode="assign_initial",
    ) -> Tuple[np.ndarray, th.Tensor, Union[th.Tensor, pd.DataFrame], np.ndarray]:
        ''' todo: to refactor
        assign the optimal theta_p and theta_s to theta_c samples.
        :param representative_theta_c: the indices of representative theta_c in theta_c clusters
        :param cluster_members: the labels in each cluster
                                (e.g. 3 clusters, [np.array([0, 4, 5)], np.array([1, 2]), np.array([3])])
        :param theta_c_samples: theta_c samples, either generated from initialization or newly generated
        :param configurations_all_subQs: the configurations of all subQs, where the length of columes is len_one_theta
        :param indices_all_subQs: an array, where columes show subQ_ids, representative/initial theta_c_ids and ids of optimal
                    theta_p solutions
        :param n_subQs: the number of subQs
        ...
        :param non_decision_tabular_features: the non_decision_tabular_features for all subQs
        :param mode: to show whether to assign the optimal theta_p and theta_s to the initial theta_c_samples or newly
                generated theta_c, using "assign_initial" or "assign_new"
        :return: mesh_theta: configurations of all subQs and all theta_c, shape (#solution, len_one_theta).
                mesh_graph_embedding: graph embeddings of all subQs and all theta_c, shape (#solution, 32)
                mesh_non_decision_tabular_features: graph embeddings of all subQs and all theta_c, shape (#solution, 28)
        ...
        if isinstance(configurations_all_subQs, np.ndarray):
            configurations_all_subQs_tensor = th.tensor(configurations_all_subQs, dtype=th.float32)

        # list, the indices are the order of subQs
        optimal_theta_p_list = [
            # list, the indices are the order of theta_c_ids in representative theta_c
            ...
            [
                configurations_all_subQs_tensor[np.where((subQ_theta_c_inds_arr == [subQ_id, theta_c_id]).all(1))].reshape(
                    -1, self.len_theta
                )
        ...
        # each element is a list, whose indices are the order of theta_c_ids in representative theta_c
        # each element shows the number of optimal theta_p solutions under a fixed theta_c and subQ
        len_opt_theta_p_per_subQ = [[x.shape[0] for x in opt_theta_p] for opt_theta_p in optimal_theta_p_list]
        ...
        if mode == "assign_initial":
            ...
            replicate_theta_c_features = [
                # list follows the number of theta_c_ids in the representative theta_c
                # each element is a tensor with shape (n_solutions, len_theta_c)
                # where n_solutions = n_cluster_members * n_optimal_theta_p
                # e.g. for subQ0, cluster0=[theta_c0, theta_c2], if optimizing representative theta_c2 gets 1
                # optimal solution, n_solutions = 2 * 1 = 2
                [
            ...
            # to assign optimal theta_p solutions to all the newly generated theta_c samples
            replicate_theta_c_features = [
                [
                    th.repeat_interleave(
                        # the settings which distinguishs the newly generated theta_c_ids,
        ...
        # concatenate to get one tensor of theta_c features for all subQs and theta_c
        replicate_theta_c_features_all_subQs = th.cat([th.cat(x) for x in replicate_theta_c_features])

        # to replicate theta_p and theta_s features based on the number of optimal theta_p solutions
        # under all theta_c and subQs
        # list follows the order of subQs, each element is a list
        replicate_theta_p_s_features = [
            ...
            # optimal solution, n_solutions = 2 * 1 = 2
            [
                opt_theta_p[:, theta_c_features_tensor.shape[1]:].repeat(
                    len(cm), 1
                )
        ...
        # concatenate to get one tensor of theta_p_s features for all subQs and theta_c
        replicate_theta_p_s_features_all_subQs = th.cat([th.cat(x) for x in replicate_theta_p_s_features])
        assert replicate_theta_c_features_all_subQs.shape[0] == replicate_theta_p_s_features_all_subQs.shape[0]
        mesh_theta = th.cat([replicate_theta_c_features_all_subQs, replicate_theta_p_s_features_all_subQs], dim=1)

        # mesh_theta_c_ids: to replicate all the theta_c_ids with its number of optimal theta_p solutions
        # each element is a list, the indices are the order of subQs
        # note: to let the theta_c in the same cluster have the same optimal theta_p solutions
        # e.g. given 2 representative theta_c (i.e. theta_c2, theta_c3), i.e. 2 theta clusters
        # where theta_c2 in the cluster0 = [theta_c0, theta_c2], theta_c3 in the cluster1 = [theta_c1, theta_c3]
        ...
        replicate_theta_c_ids = [
            # list: it follows the order of theta_c_ids in the representative theta_c
            # e.g. [theta_c2, theta_c3] in the above example
            # each element is a tensor with theta_c_ids replicated by its number of optimal theta_p solutions,
            # for all theta_c within the same cluster
            # e.g. theta_c2 has 1 and theta_c3 has 2 optimal theta_p solutions respectively
        ...
        # list: it follows the order of subQs, each element is a tensor, with shape(n_solutions, )
        # note: after concatenating, n_solutions could be different for different subQs
        # e.g. given a query with 2 subQs, theta_c0, there are 1 optimal solution in subQ0, and 2 in subQ1
        # replicate_theta_c_subQs would be [tensor(1, ), tensor(2, )]
        replicate_theta_c_subQs = [th.cat(x) for x in replicate_theta_c_ids]
        # replicate subQ_ids based on the number of solutions in replicate_theta_c_subQs
        # The aim is to let the subQ_ids align with the number of solutions in each subQ
        ...
        if use_ag:
            ...
            mesh_non_decision_tabular_features = pd.concat(
                # a list follows the order of subQs,
                # each element is a pd.Dataframe with replicated non_decision tabular features for each subQ
                [
        ...
        ########### update the array of indices (subQ_ids, theta_c_ids, and optimal theta_p_s ids)
        ...
        return mesh_theta, mesh_graph_embedding, mesh_non_decision_tabular_features, indices_arr
    ...
    def estimate_opt_theta_p_s(
        ...
        # fixme: to double-check
        '''
        estimate the objective values of all theta_c using the optimal theta_p and theta_s of representative theta_c
        :param representative_theta_c: the indices of representative theta_c in theta_c clusters
        :param init_n_c_samples: the number of initial theta_c samples, used in assigning the optimal theta_p for
                        the new theta_c samples,
                        - as to distinguish new theta_c and the initial theta_c samples, the indices of new theta_c
                        sums the number of initial theta_c samples

        :param cluster_members: the labels in each cluster
                                (e.g. 3 clusters, [np.array([0, 4, 5)], np.array([1, 2]), np.array([3])])
        :param theta_c_samples: theta_c samples, either generated from initialization or newly generated
        :param configurations_all_subQs: the configurations of all subQs, where the length of columes is len_one_theta
        :param indices_all_subQs: an array, where columes show subQ_ids, theta_c_ids and ids of optimal
                    theta_p solutions
        :param n_subQs: the number of subQs
        :param use_ag: flag to show whether using AutoGluon in predictive models
        :param graph_embeddings: the graph embeddings for all subQs
        ...
        :param mode: to show whether to assign the optimal theta_p and theta_s to the initial theta_c_samples or newly
                generated theta_c, using "assign_initial" or "assign_new"
        :return: obj_values_all_subQs: objective values of all_subQs and all_theta_c with optimal theta_p and theta_s
                            of representative theta_c, shape (#solution, n_objs)
                mesh_theta: configurations of all subQs and all theta_c, shape (#solution, len_one_theta)
                updated_indices_all_subQs: the indices of subQs, all theta_c and the optimal theta_p
                model_inference_arr: the array of model inference info, where columes show [n_evals, model_inference_time]
                    e.g. n_evals is equal to the number of predictions/input from the model
        '''

        (mesh_theta, mesh_graph_embedding, mesh_non_decision_tabular_features, updated_indices_all_subQs), \
    ...
    def union_opt_theta_c(
        ...
        # fixme: to double-check
        '''
        union the optimal theta_c (i.e. from the subQ-level solutions without constraints of identical theta_c) of all
         subQs to provide a warm start to generate new theta_c
        :param n_subQs: the number of subQs
        :param len_theta_c: the length of the theta_c, i.e. 8
        :param objective_values_all_subQs: objective values of all_subQs and all_theta_c with optimal theta_p and theta_s
                            of representative theta_c, shape (#solution, n_objs)
        :param configurations_all_subQs: configurations of all subQs and all theta_c, shape (#solution, len_one_theta)
        :param indices_all_subQs_all_theta_c: the indices of subQs, all theta_c and the optimal theta_p
        :return: the optimal theta_c from all subQs
        '''

        local_opt_theta_c_list = []
        for subQ_id in range(n_subQs):
            subQ_values_inds = np.where(indices_all_subQs_all_theta_c[:, 0] == subQ_id)[0].tolist()
    ...
    def crossover(
        ...
        matching_rows_opt_c = np.all(
            uniq_new_c[:, None, :] == np.array(union_opt_theta_c_list)[None, :, :], axis=-1
        )
    ...
    def assign_new_theta_c_to_clusters(
        ...
        :param cluster_model: cluster models, which takes theta_c samples as input, and returns cluster labels
        :return: new_cluster_members: labels of theta_c clusters,
                new_representative_theta_c: representative_theta_c of the new cluster labels
        '''
    ...
    def union_initial_and_new_solution_sets(
        ...
        '''
        Union the solutions sets from initial theta_c samples and newly generated theta_c_samples
        :param obj_values_all_subQs_init_theta_c: the objective values of all subQs from the initial theta_c samples
        :param obj_values_all_subQs_new_theta_c: the objective values of all subQs from the newly generated theta_c
        :param config_all_subQs_init_theta_c: the configurations of all subQs from the initial theta_c samples
        :param config_all_subQs_new_theta_c: the configurations of all subQs from the newly generated theta_c
        :param indices_all_subQs_init_theta_c: the indices of subQs, initial theta_c samples and optimal theta_p.
        :param indices_all_subQs_new_theta_c: the indices of subQs, new theta_c samples and optimal theta_p.
        :param model_inference_info_optimize_theta_p: the model inference information (n_evaluation, time cost)
                                                        in optimizing theta_p and theta_s for each theta_c of each subQ
        :param model_inference_info_assign_init_theta_c: the model inference information (n_evaluation, time cost)
                                                        in assigning optimal theta_p and theta_s to each initial theta_c
                                                        of each subQ
        :param model_inference_info_assign_new_theta_c: the model inference information (n_evaluation, time cost)
                                                        in assigning optimal theta_p and theta_s to each new theta_c
                                                        of each subQ
        :return: obj_values_all_subQs: the subQ-level Pareto optimal solutions, which are Pareto optimal under each
                            theta_c and subQ, shape (#total_solutions, n_objs), here n_objs=2 with latency and cost.
                config_all_subQs: the corresponding configurations, shape (#total_solutions, len_one_theta), here
        ...
        if isinstance(config_all_subQs_init_theta_c, np.ndarray):
            config_all_subQs = np.concatenate([config_all_subQs_init_theta_c,
                                               config_all_subQs_new_theta_c])
        else:
            assert isinstance(config_all_subQs_init_theta_c, th.Tensor)
            config_all_subQs = th.concatenate([config_all_subQs_init_theta_c,
                                               config_all_subQs_new_theta_c])
        obj_values_all_subQs = np.concatenate([obj_values_all_subQs_init_theta_c,
                                               obj_values_all_subQs_new_theta_c])
        indices_all_subQs = np.concatenate([indices_all_subQs_init_theta_c,
                                            indices_all_subQs_new_theta_c])
