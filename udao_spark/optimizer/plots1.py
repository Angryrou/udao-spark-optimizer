...
def plot_err_bar(
    ...
    if isinstance(queries, str):
        ...
        ax2.set_title(f"All TPCH queries with initial theta ({sample_mode}, {cpu_mode})")
    else:
    ...
    # plot_path = f"./output/plots/grids/HV/"
    # if not os.path.exists(plot_path):
    #     os.makedirs(plot_path, exist_ok=True)
    # plt.savefig(f"{plot_path}/c_{n_c_samples}_p{n_p_samples}.pdf")


def plot_box(X, methods, sample_mode, plot_mode="comp_all"):
    x = np.array(X).T
    ...
    if "dag_opt" in plot_mode:
        ...
        plt.xticks(rotation=90)
    ax.set_title(f"All TPCH queries for initial theta ({sample_mode})")
    plt.tight_layout()
    plt.show()

def pareto_frontier(queries, stages, cpu_mode, sample_mode):
    ...
    for query_id, n_stages in zip(queries, stages):
        ...
        ax.set_title(f"TPCH {query_id}_n_subQ_{n_stages} ({cpu_mode})")
        ax.legend()
        # plt.show()
...
def pareto_frontier_all_fine_control(queries, stages, cpu_mode, sample_mode, div_moo_setting,
                          evo_setting, ws_setting, ppf_setting, save_data_header, is_query_control, is_oracle):
    ...
    for query_id, n_s in zip(queries, stages):
        ...
        for method in methods:
            if "div_and_conq_moo" in method:
                ...
                # data_path = f"./output/output/test/latest_model_{cpu_mode}/ag/{method}/100_1000/time_-1/query_{query_id}_n_{n_stages}"
                data_path = f"{save_data_header}/query_control_{is_query_control}/latest_model_{cpu_mode}/ag/oracle_{is_oracle}/{method}/{evo_setting}/time_-1/query_{query_id}_n_{n_s}"

                F = np.loadtxt(f"{data_path}/F.txt")
                F_list.append(F)
            elif method == "ws":
                # data_path = f"./output/output/test/latest_model_{cpu_mode}/ag/{method}/1000000_11/time_-1/query_{query_id}_n_{n_stages}"
                data_path = f"{save_data_header}/query_control_{is_query_control}/latest_model_{cpu_mode}/ag/oracle_{is_oracle}/{method}/{ws_setting}/time_-1/query_{query_id}_n_{n_s}"
                F = np.loadtxt(f"{data_path}/F.txt")
                F_list.append(F)
            elif method == "ppf":
                # data_path = f"./output/test/query_control_False/latest_model_{cpu_mode}/ag/{method}/1_2_2/time_-1/query_{query_id}_n_{n_stages}"
                data_path = f"{save_data_header}/query_control_{is_query_control}/latest_model_{cpu_mode}/ag/oracle_{is_oracle}/{method}/{ppf_setting}/time_-1/query_{query_id}_n_{n_s}"
        ...
        ax.set_xlabel("Query cost")
        ax.set_ylabel("Query latency")
        ax.set_title(f"TPCH {query_id}_n_subQ_{n_s} ({cpu_mode})")
        ax.legend()
        # plt.show()
...
def comp_pareto_frontier_query_fine_control(queries, stages, cpu_mode, sample_mode, div_moo_setting,
                          evo_setting, ws_setting, ppf_setting, save_data_header, is_oracle):
    ...
    for query_id, n_s in zip(queries, stages):
        ...
        for method in methods:
            ...
            if "div_and_conq_moo" in method:
                ...
                # data_path = f"./output/output/test/latest_model_{cpu_mode}/ag/{method}/100_1000/time_-1/query_{query_id}_n_{n_stages}"
                data_path_fine = f"{save_data_header}/query_control_False/latest_model_{cpu_mode}/ag/oracle_{is_oracle}/{method}/{evo_setting}/time_-1/query_{query_id}_n_{n_s}"
                data_path_query = f"{save_data_header}/query_control_True/latest_model_{cpu_mode}/ag/oracle_{is_oracle}/{method}/{evo_setting}/time_-1/query_{query_id}_n_{n_s}"
                ...
                # data_path = f"./output/test/query_control_False/latest_model_{cpu_mode}/ag/{method}/1_2_2/time_-1/query_{query_id}_n_{n_stages}"
                data_path_fine = f"{save_data_header}/query_control_False/latest_model_{cpu_mode}/ag/oracle_{is_oracle}/{method}/{ppf_setting}/time_-1/query_{query_id}_n_{n_s}"
                data_path_query = f"{save_data_header}/query_control_True/latest_model_{cpu_mode}/ag/oracle_{is_oracle}/{method}/{ppf_setting}/time_-1/query_{query_id}_n_{n_s}"
                F_fine = np.loadtxt(f"{data_path_fine}/F.txt")
                F_query = np.loadtxt(f"{data_path_query}/F.txt")
...
def compare_diff_dag_opt(queries, stages, cpu_mode, sample_mode, div_moo_setting, save_data_header="",
    ...
    for query_id, n_s in zip(queries, stages):
        ...
        for i, method in enumerate(methods):
            ...
            if save_pareto_front:
                ax.scatter(F[:, 1], F[:, 0], marker=markers[i], color=colors[i], label=f"{dag_opt_algo} ({F_uniq.shape[0]})")
                ax.legend()
                ax.set_xlabel("Query Cost")
                ax.set_ylabel("Query Latency")
        ...
        # all_nadir = F_arr.max(0)
        # all_utopia = F_arr.min(0)
    ...
    plot_err_bar(hv_mean, hv_std, methods, queries, sample_mode, mode="hv", cpu_mode=cpu_mode, plot_mode="comp_dag_opt")
    # plot_err_bar(tc_mean, tc_std, algo_labels, queries, sample_mode, mode="time", cpu_mode=cpu_mode)
    plot_box(all_tc_list, methods, sample_mode, plot_mode="comp_dag_opt")
...
def compare_approx_seq_div(queries, stages):
    fig, ax = plt.subplots()
    ...
    ax.set_ylabel("Query Latency")
    if len(queries) == 1:
        ax.set_title(f"TPCH query {queries[0]} with {stages[0]} subQs")
    else:
        ax.set_title(f"All {len(queries)} TPCH queries")
    ax.legend()
    plt.show()
    plt.tight_layout()
...
def a_test_moo():
    ...
    ax.scatter(cost, latency, marker="o", color="blue", edgecolors="blue", alpha=0.3, label="Dominated")

    q_pareto_flag = is_pareto_efficient(query)
    q_pareto = query[q_pareto_flag]
    lat_q_pareto, cost_q_pareto = q_pareto[:, 0], q_pareto[:, 1]
    ax.scatter(cost_q_pareto, lat_q_pareto, marker="o", color="red", edgecolors="red", alpha=0.6, label="Pareto")
