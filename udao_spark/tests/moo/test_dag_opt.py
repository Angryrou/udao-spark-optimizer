...
class TestDAGOpt:
    def test_process_input(self, dag_opt: DAGOpt) -> None:
        ...
        (result_sorted_objective_values, result_sorted_configs, result_sorted_indices), \
        ...
        assert all([[np.all(result == expect) for result, expect in zip(result_subQ, expect_subQ)]
                for result_subQ, expect_subQ in zip(result_sorted_objective_values,
                                          sorted_obj_values_all_subQs)])
        assert all([[np.all(result == expect) for result, expect in zip(result_subQ, expect_subQ)]
                for result_subQ, expect_subQ in zip(result_sorted_configs,
                                          sorted_configs_all_subQs)])
    ...
    def test_get_solutions_with_sorted_subQ_ids(self, dag_opt: DAGOpt) -> None:
        ...
        (sorted_obj_values_with_split_subQs, sorted_configs_with_split_subQs, sorted_indices),\
        _ = dag_opt.get_solutions_with_sorted_subQ_ids(
            objective_values,
            configurations,
            indices
        )
        ...
        assert all([np.all(result == expect) for result, expect in zip(sorted_obj_values_with_split_subQs,
                                                                      expect_sorted_obj_values_with_split_subQs)])
        assert all([np.all(result == expect) for result, expect in zip(sorted_configs_with_split_subQs,
                                                                       expect_sorted_configs_with_split_subQs)])
    ...
    def test_split_subQs_set_with_theta_c(self, dag_opt: DAGOpt) -> None:
        ...
        assert all([[np.all(result == expect) for result, expect in zip(result_subQ, expect_subQ)]
                    for result_subQ, expect_subQ in zip(sorted_obj_values_all_subQs_all_theta_c,
                                                        sorted_obj_values_all_subQs)])
        assert all([[np.all(result == expect) for result, expect in zip(result_subQ, expect_subQ)]
                    for result_subQ, expect_subQ in zip(sorted_configs_all_subQs_all_theta_c,
                                                        sorted_configs_all_subQs)])
    ...
    def test_pad_solutions_one_subQ(self, dag_opt: DAGOpt) -> None:
        ...
        (obj_values_one_subQ_pad_list, configs_one_subQ_pad_list), tc_pad = dag_opt.pad_solutions_one_subQ(
            n_max_solutions,
            obj_values_one_subQ,
            configs_one_subQ,
        )

        expect_obj_values_one_subQ_pad_list = [[25, 20], [23, 23], [23, 25], [9223372036854775807, 9223372036854775806]]
        expect_configs_one_subQ_pad_list = [[9, 4, 26, 27, 34, 1, 2], [9, 4, 37, 25, 33, 1, 2],
                                            [12, 16, 37, 25, 33, 1, 2], [-1, -1, -1, -1, -1, -1, -1]]

        assert np.all(np.array(obj_values_one_subQ_pad_list == expect_obj_values_one_subQ_pad_list))
        assert np.all(np.array(configs_one_subQ_pad_list == expect_configs_one_subQ_pad_list))
    ...
    def test_ws_per_subQ(self, dag_opt: DAGOpt) -> None:
        ...
        expect_select_configs_arr = np.array([[9, 4, 26, 27, 34, 1, 2, 9, 4, 37, 25, 33, 1, 2],
    ...
    def test_ws_all_subQs(self, dag_opt: DAGOpt) -> None:
        ...
        expect_query_configs = [
            np.array([[9, 4, 26, 27, 34, 1, 2], [12, 16, 37, 25, 33, 1, 2],
                      [9, 4, 37, 25, 33, 1, 2], [12, 16, 37, 25, 33, 1, 2]]),
            np.array([[9, 4, 26, 27, 34, 1, 2], [12, 16, 37, 25, 33, 1, 2],
                      [9, 4, 26, 27, 34, 1, 2], [12, 16, 37, 25, 33, 1, 2]]),
            np.array([[9, 4, 37, 25, 33, 1, 2], [12, 16, 26, 27, 34, 1, 2],
                      [9, 4, 26, 27, 34, 1, 2], [12, 16, 26, 27, 34, 1, 2]])
        ]

        assert np.all(query_obj_values == expect_query_obj_values)
        assert all([np.all(result == expect) for result, expect in zip(query_configs, expect_query_configs)])
    ...
    def test_filter_dominated_ws_based(self, dag_opt: DAGOpt) -> None:
        ...
        (pareto_query_obj_values, pareto_query_configs), _ = dag_opt.filter_dominated_ws_based(
            query_obj_values,
            query_configs
        )
        ...
        expect_pareto_query_configs = np.array([
            [12, 16, 37, 25, 33, 1, 2, 12, 16, 37, 25, 33, 1, 2, 12, 16, 26, 27, 34, 1, 2],
            [9, 4, 37, 25, 33, 1, 2, 9, 4, 26, 27, 34, 1, 2, 9, 4, 26, 27, 34, 1, 2],
            [9, 4, 26, 27, 34, 1, 2, 9, 4, 26, 27, 34, 1, 2, 9, 4, 37, 25, 33, 1, 2]])
    ...
    def test_compute_merged_values(self, dag_opt: DAGOpt) -> None:
        subQ1_id = 0
        subQ2_id = 1
        subQ1 = (sorted_obj_values_all_subQs[subQ1_id], sorted_configs_all_subQs[subQ1_id])
        subQ2 = (sorted_obj_values_all_subQs[subQ2_id], sorted_configs_all_subQs[subQ2_id])

        (merged_obj_values_all_theta_c, merged_configs_indices_all_theta_c), _ = dag_opt.compute_merged_values(
            subQ1,
            subQ2,
        )
        expect_merged_obj_values_all_theta_c = [[[47.0, 44.0], [45.0, 47.0]], [[44.0, 51.0]]]
        expect_merged_configs_indices_all_theta_c = [[(0, 0), (1, 0)], [(0, 0)]]

        assert all([np.all(np.array(result) == np.array(expect)
                           for result, expect in zip(merged_obj_values_all_theta_c,
                                                     expect_merged_obj_values_all_theta_c))])
        assert all([np.all(np.array(result) == np.array(expect)
                           for result, expect in zip(merged_configs_indices_all_theta_c,
                                                     expect_merged_configs_indices_all_theta_c))])

    def test_filter_dominated_merged_values(self, dag_opt: DAGOpt) -> None:
        # NOTE: not the same as the expect_merged_obj_values_all_theta_c in test_compute_merged_values
        merged_obj_values_all_theta_c = [[[47.0, 44.0], [45.0, 43.0]], [[44.0, 51.0]]]
        merged_configs_indices_all_theta_c = [[(0, 0), (1, 0)], [(0, 0)]]

        (pareto_merged_obj_values, pareto_merged_configs_indices), _ = dag_opt.filter_dominated_merged_values(
            merged_obj_values_all_theta_c,
            merged_configs_indices_all_theta_c
        )

        expect_pareto_merged_obj_values = [np.array([[45.0, 43.0]]), np.array([[44.0, 51.0]])]
        ...
        assert all([np.all(result == expect) for result, expect in zip(pareto_merged_configs_indices,
                                                                       expect_pareto_merged_configs_indices)])
    ...
    def test_update_merged_configs_with_indices_choices(self, dag_opt: DAGOpt) -> None:
        ...
        merged_configs_all_theta_c, _ = dag_opt.update_merged_configs_with_indices_choices(
        ...
        assert all([np.all(result == expect) for result, expect in zip(merged_configs_all_theta_c,
                                                                       expect_merged_configs_all_theta_c)])

    def test_merge_two_subQs(self, dag_opt: DAGOpt) -> None:
        subQ1_id = 0
        subQ2_id = 1
        stack_obj_values = [sorted_obj_values_all_subQs[subQ1_id],
                            sorted_obj_values_all_subQs[subQ2_id]]
        stack_configs = [sorted_configs_all_subQs[subQ1_id],
        ...
        expect_pareto_merged_obj_values = [np.array([[47.0, 44.0], [45.0, 47.0]]),
                                           np.array([[44.0, 51.0]])]
    ...
    def test_solve_more_than_two_subQs(self, dag_opt: DAGOpt) -> None:
        ...
        (
        updated_stack_obj_values, updated_stack_configs), _ = dag_opt.solve_more_than_two_subQs(
            subQs_str,
            stack_obj_values,
            stack_configs,
        )
        ...
        assert all([[np.all(result == expect) for result, expect in zip(result_subQ, expect_subQ)]
                    for result_subQ, expect_subQ in zip(updated_stack_obj_values,
                                                        expect_updated_stack_obj_values)])
        assert all([[np.all(result == expect) for result, expect in zip(result_subQ, expect_subQ)]
                    for result_subQ, expect_subQ in zip(updated_stack_configs,
                                                        expect_updated_stack_configs)])

    def test_traverse_all_subQs_non_recur(self, dag_opt: DAGOpt) -> None:
        (pareto_query_obj_values, pareto_query_configs), _ = dag_opt.traverse_all_subQs_non_recur(
            subQs,
            sorted_obj_values_all_subQs,
            sorted_configs_all_subQs,
        )

        expect_pareto_query_obj_values = [np.array([[72.0, 74.0], [75.0, 65.0], [73.0, 68.0]]),
                                          np.array([[69.0, 80.0]])]
        expect_pareto_query_configs = [
            np.array([[9, 4, 37, 25, 33, 1, 2, 9, 4, 26, 27, 34, 1, 2, 9, 4, 26, 27, 34, 1, 2],
                      [9, 4, 26, 27, 34, 1, 2, 9, 4, 26, 27, 34, 1, 2, 9, 4, 37, 25, 33, 1, 2],
                      [9, 4, 37, 25, 33, 1, 2, 9, 4, 26, 27, 34, 1, 2, 9, 4, 37, 25, 33, 1, 2]]),
            np.array([[12, 16, 37, 25, 33, 1, 2, 12, 16, 37, 25, 33, 1, 2, 12, 16, 26, 27, 34, 1, 2]])]

        assert all([np.all(result == expect) for result, expect in zip(pareto_query_obj_values,
                                                                       expect_pareto_query_obj_values)])
        assert all([np.all(result == expect) for result, expect in zip(pareto_query_configs,
                                                                       expect_pareto_query_configs)])
    ...
    def test_filter_query_dominated_general_div_and_conq(self, dag_opt: DAGOpt) -> None:
        # NOTE: it is not the same as the expect_query_obj_values_list in test_traverse_all_subQs_non_recur
        query_obj_values_list = [np.array([[72.0, 74.0], [75.0, 65.0], [71.0, 68.0]]),
                                          np.array([[69.0, 80.0]])]
        query_configs_list = [
            np.array([[9, 4, 37, 25, 33, 1, 2, 9, 4, 26, 27, 34, 1, 2, 9, 4, 26, 27, 34, 1, 2],
                      [9, 4, 26, 27, 34, 1, 2, 9, 4, 26, 27, 34, 1, 2, 9, 4, 37, 25, 33, 1, 2],
        ...
        (pareto_query_obj_values, pareto_query_configs), _ = dag_opt.filter_query_dominated_general_div_and_conq(
            query_obj_values_list,
            query_configs_list,
        )

        expect_pareto_query_obj_values = np.array([[69.0, 80.0], [71.0, 68.0], [75.0, 65.0]])
        expect_pareto_query_configs = np.array([
            [12, 16, 37, 25, 33, 1, 2, 12, 16, 37, 25, 33, 1, 2, 12, 16, 26, 27, 34, 1, 2],
            [9, 4, 37, 25, 33, 1, 2, 9, 4, 26, 27, 34, 1, 2, 9, 4, 37, 25, 33, 1, 2],
            [9, 4, 26, 27, 34, 1, 2, 9, 4, 26, 27, 34, 1, 2, 9, 4, 37, 25, 33, 1, 2]])
    ...
    def test_compute_boundary_all_subQs(self, dag_opt: DAGOpt) -> None:
        ...
        assert all([np.all(result == expect) for result, expect in zip(boundary_obj_values_all_subQs,
                                                                       expect_boundary_obj_values_all_subQs)])
        assert all([np.all(result == expect) for result, expect in zip(boundary_configs_all_subQs,
                                                                       expect_boundary_configs_all_subQs)])
    ...
    def test_construct_query_solutions(self, dag_opt: DAGOpt) -> None:
        ...
        expect_boundary_query_objs = np.array(
            [[49, 51], [52, 47], [46, 54], [51, 46]]
        )
        ...
        assert np.all(boundary_configs_all_subQs_arr == expect_boundary_configs_all_subQs_arr)

    def test_filter_dominated_boundary_based(self, dag_opt: DAGOpt) -> None:
        boundary_query_objs = np.array(
            [[49, 51], [52, 47], [46, 54], [51, 46]]
        )
