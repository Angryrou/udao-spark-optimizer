...
from udao.model.embedders.layers.iso_bn import IsoBN
...
class BaseGraphEmbedder(BaseEmbedder, ABC):
    ...
    class Params(BaseEmbedder.Params):
        ...
        """The dimension of the operation histogram embedding."""
        bitmap_embedding_dim: Optional[int]
        """The dimension of the operation bitmap embedding."""
    ...
    def __init__(self, net_params: Params) -> None:
        ...
        self.op_hist = "hist" in op_groups
        self.op_bitmap = "bitmap" in op_groups
        if self.op_type:
        ...
        if self.op_hist:
            if net_params.hist_embedding_dim is None:
                raise ValueError(
                    "hist_embedding_dim must be provided "
                    "if `hist` is included in op_groups"
                )
            # fixme: 150 is hardcoded for encoding 3 predicates with 50 equi-height bins
            self.op_hist_embedder = nn.Linear(150, net_params.hist_embedding_dim)
            self.input_size += net_params.hist_embedding_dim
        if self.op_bitmap:
            # fixme: 1000 is hardcoded with each table sampled 1000 rows w/ replacement.
            if net_params.bitmap_embedding_dim is None:
                raise ValueError(
                    "bitmap_embedding_dim must be provided "
                    "if `bitmap` is included in op_groups"
                )
            self.op_bitmap_embedder = nn.Linear(1000, net_params.bitmap_embedding_dim)
            self.input_size += net_params.bitmap_embedding_dim
