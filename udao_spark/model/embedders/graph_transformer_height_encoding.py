...
class GraphTransformerHeightEncodingSuperNode(BaseGraphEmbedder):
    ...
    def __init__(self, net_params: Params) -> None:
        ...
        if self.attention_layer_name in ["QF", "GTN_HE"]:
            if net_params.max_dist is None:
                raise ValueError("max_dist is required for QF and GTN_HE")
            if net_params.max_height is None:
                raise ValueError("max_height is required for QF and GTN_HE")
            if self.readout != "mean":
                raise ValueError(
                    "readout must be mean for QF and GTN_HE to make sign consistent"
                )
